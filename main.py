...
async def call_vector_api_simple(file_content: bytes, filename: str, params: dict):
    """Simple API call for Vector Drawing API without complex retry dependencies"""
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt == MAX_RETRIES - 1:
                raise HTTPException(
                    status_code=503, 
                    detail=f"Vector API connection failed: {str(e)}"
                )
...
async def process_pdf(
    ...
    try:
        ...
        try:
            vector_response = await call_vector_api_simple(file_content, file.filename, params)
        ...
        try:
            ...
            # Double-check if we need to parse again
...
async def root():
    ...
    return {
        "title": "Master API",
        "version": "1.0.6",
        "description": "Simple PDF processing through Vector Drawing API and Pre-Filter API",
        "endpoints": {
            "/": "This page",
        ...
        "improvements": {
            "simplified_dependencies": "No urllib3 retry conflicts",
            "vision_output_support": "Accepts and processes vision output",
            "robust_parsing": "Handles various JSON response formats",
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"Starting Simple Master API on port {PORT}")
    logger.info("Features: Vision output support, simple error handling, no dependency conflicts")
